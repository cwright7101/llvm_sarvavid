...
//   const auto &FAM = AM.getResult<FunctionAnalysisManagerLoopProxy>(L, AR).getManager();
...
bool SarvOpts::Optimizations::loopInvarCodeMotion(llvm::Loop *L, llvm::AliasAnalysis *AA,
                                      ...
                                      bool DeleteAST)
{
  ...
  assert(L->isLCSSAForm(*DT) && "Loop is not in LCSSA form.");
  // llvm::errs()<<"is recursively lcssa ?: "<<L->isRecursivelyLCSSAForm(*DT, *LI)<<"\n";
  // llvm::errs()<<"is safe to clone? : "<<L->isSafeToClone()<<"\n";
  // auto lStuff = L->getLoopID();
  // if(lStuff)
  // 	llvm::errs()<<"lStuff is not null\n";
  // else
  // 	llvm::errs()<<"lStuff is null\n";


  // llvm::AliasSetTracker *CurAST = collectAliasInfoForLoop(L, LI, AA);
  ...
  // if (Preheader){
  //   // Changed |= hoistRegion(DT->getNode(L->getHeader()), AA, LI, DT, TLI, L, CurAST, &SafetyInfo, ORE, true);
  // }

  // // assert(L->isLCSSAForm(*DT) && "Loop not left in LCSSA form after LICM!");
  // // assert((!L->getParentLoop() || L->getParentLoop()->isLCSSAForm(*DT)) && "Parent loop not left in LCSSA form after LICM!");
