...
//    https://github.com/tingsu/Stoat.git
//    lookups1s3 = stream_lookup2(kmer_s1,s3,1);
//    
//    kmer_s2 = k_merize2(s2,act_S2,W,1);
//    
//    lookups2s3 = stream_lookup2(kmer_s2,s3,1);
    string query_word;
    long interval;
    long status;
    ...
    for(it1 in lookups1s3){
        ...
        vt_ug1 = similarity_computation("ungapped",q1l,db1l,s1,s3,act_S1,act_S3);
    ...
    for(it2 in lookups2s3) {
        ...
        vt_ug2 = similarity_computation("ungapped",q2l,db2l,s2,s3,act_S2,act_S3);
